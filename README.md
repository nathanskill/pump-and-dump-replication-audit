...
**1. Zenodo (primary; same-day, no gate).** Sign in at zenodo.org with GitHub, enable this repository under Settings → GitHub, then publish a GitHub release of tag `v0.2.1-results-freeze` — a results-complete tag whose tree contains the frozen protocol, all `artifacts/formal_*` and `artifacts/a4_addendum_*` directories cited in the manuscript, `src/`, and `CITATION.cff`. Zenodo archives the release automatically and reads `CITATION.cff`. Paste the resulting DOI badge here and into the manuscript's Data-availability statement.

- Two earlier tags are **not** the archival target. `v0.1.0-protocol-freeze` at `c2736ed` is the protocol anchor only; it predates the results. `v0.2.0-results-freeze` at `faf7767` carries the same results artifacts as `v0.2.1` but its tree still contains the superseded `CITATION.cff` naming `version: v0.1.0-protocol-freeze` — releasing from it would make Zenodo mint the record under the wrong version. `v0.2.1-results-freeze` exists precisely to fix that without rewriting a published tag. **Neither earlier tag is moved or deleted:** in this project a published tag is a record, and the discipline is to supersede, not to overwrite.
