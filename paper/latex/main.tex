...
% Long artifact directory names set in \texttt are unbreakable and
% overflow the LNCS text block. Permit a line break after each text-mode
% underscore. Math-mode subscripts are unaffected.
\let\origtextunderscore\textunderscore
\renewcommand{\textunderscore}{\origtextunderscore\allowbreak}
...
\else
% \href needs hyperref, which this project does not load (loading it late
% shifts LNCS float placement). \url comes from the url package, already
% loaded, and prints the same address.
DOI: \url{https://doi.org/10.5281/zenodo.21805707}.
\fi
...
verified against hash-recorded artifacts.
% The bracketed note "Adapt to venue disclosure format at submission" stood
% here until the venue was fixed. FC 2027 sets no prescribed wording for
% AI-assistance disclosure, so the paragraph stands as written and the note
% is discharged. Springer's mandatory "Disclosure of Interests" heading is a
% camera-ready obligation and is added there, not here.
