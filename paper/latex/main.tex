...
nonetheless degrades sharply with aggregation frequency: at the primary
forward checkpoint the event detection rate at the released threshold
falls from 0.922 at 25\,s to 0.641 at 5\,s, a weakness that row-level
scores alone do not reveal; ablating the four time-of-day features
lowers forward average precision by only 0.010--0.028. Taken together,
the evaluation-design choice the literature scrutinises most --- the
split family --- does not materially inflate discrimination here, while
one it scrutinises less --- the aggregation frequency, read at event
level --- does. All conclusions are restricted to the event-centred
public benchmark; no real-market prevalence, false-alert or deployment
claim is made. Studies that reuse this artifact should report
event-exclusive and event-level metrics alongside row-level scores: on
this benchmark the added cost is near zero, and the information gained
at 5\,s is substantial.
...
This paper asks one question: \textbf{does the choice of evaluation
family change what this benchmark says --- and if so, which metric
reveals it?} Contributions:
...
\item \textbf{The published Table~III cannot be recovered from
currently public materials.} We record the parameter, code and matrix
differences and leave their source open
(Section~\ref{sec:tableiii}).
\item \textbf{Discrimination is insensitive to the split family on an
identical 326-event universe; event-level detection is not insensitive
to frequency.} Established under a pre-frozen event-level outcome
...
2026, with the results of this study already in hand. No response had
been received at the time of submission; this is recorded as a fact
about the correspondence attempt only, and implies nothing about the
original work. The status will be re-checked before any camera-ready
version.
...
\texttt{min\_\allowbreak samples\_\allowbreak leaf=1}, \texttt{random\_state=1}). The
paper-described configuration (\texttt{max\_depth=4},
\texttt{min\_\allowbreak samples\_\allowbreak leaf=6}) is retained as a traceability
...
\texttt{gate0\_\allowbreak upstream\_\allowbreak stdout\_\allowbreak capture\_\allowbreak 20260721},
\texttt{gate0\_\allowbreak released\_\allowbreak code\_\allowbreak rf\_\allowbreak all\_\allowbreak v2\_\allowbreak 20260721},
\texttt{gate0\_\allowbreak baseline\_\allowbreak comparison\_\allowbreak 20260721});
...
\texttt{gate0\_\allowbreak paper\_\allowbreak described\_\allowbreak rf\_\allowbreak all\_\allowbreak 20260721}); see
...
\texttt{a4\_\allowbreak addendum\_\allowbreak v1\_\allowbreak 20260723}). Cross-frequency AP differences
...
\texttt{formal\_\allowbreak s1\_\allowbreak s0\_\allowbreak v1\_\allowbreak 20260722},
\texttt{formal\_\allowbreak forward\_\allowbreak v1\_\allowbreak 20260722}.)
...
80\%. (Artifacts: \texttt{formal\_\allowbreak forward\_\allowbreak v1\_\allowbreak 20260722}.)

Per protocol, the full frozen outcome set is reported in analysis
addendum A4 (artifact \texttt{a4\_\allowbreak addendum\_\allowbreak v1\_\allowbreak 20260723}): EDR@30 =
...
\texttt{formal\_\allowbreak forward\_\allowbreak ablation\_\allowbreak v1\_\allowbreak 20260722}).
...
from the pinned upstream inputs by \texttt{src/run\_\allowbreak formal\_\allowbreak cv.py} and
\texttt{src/run\_\allowbreak formal\_\allowbreak forward.py}, and their SHA-256 hashes are
recorded in
\texttt{artifacts/a4\_\allowbreak addendum\_\allowbreak v1\_\allowbreak 20260723/artifact\_\allowbreak manifest.csv}
for verification. A results-complete release (tag
\texttt{v0.2.1-results-freeze}, whose tree contains the frozen
...
providing a tamper-evident anchor independent of the
rewritten Git history and of the upstream repository's continued
availability.
% NOTE: do NOT insert a real DOI in the anonymized build, and do not name
% the archive: the deposit record names the author. FC 2027 expressly
% permits non-anonymous preprints, so naming the platform would not breach
% the CFP -- but withholding it costs nothing and removes a free lookup.
% See ANONYMIZATION_NOTES.md.
\ifanonymous
[{\itshape Archive DOI withheld for anonymous review.}]
\else
DOI: \href{https://doi.org/10.5281/zenodo.21805707}{10.5281/zenodo.21805707}.
\fi
...
\texttt{formal\_\allowbreak forward\_\allowbreak ablation\_\allowbreak v1\_\allowbreak 20260722} (full model:
\texttt{formal\_\allowbreak forward\_\allowbreak v1\_\allowbreak 20260722}). See
